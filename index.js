/*
	...
	あ GK KASI CREDIT YT Bambang OFC, GK USAH RE UPLOAD
	...
	あ JASA RUN HEROKU BISA PC wa.me/6282313021398 BIAR BOT NYA GAK TERLALU DELAY
...
apiku = 'https://Bambangofc.github.io/'
gc_wa_lu = 'https://chat.whatsapp.com/EU890BcXjyBDkNaUT5WmY' //klo gk punya gc wa gk usah di ganti 👍
...
await alpha.updatePresence(from, Presence.composing)
	...
	return admins
		...
		const alphaNumber = [`6282313021398@s.whatsapp.net`, `6282313021398@s.whatsapp.net` , `6282313021398@s.whatsapp.net`]
...
let _votes = JSON.parse(fs.readFileSync(`./lib/vote/${from}.json`))  
let fil = vote.map(v => v.participant)
let id_vote = sender ? sender : '6282313021398@s.whatsapp.net'
if(fil.includes(id_vote)) {
...
/////
 for (let Bambang of setiker){
	if (budy === Bambang){
		result = fs.readFileSync(`./media/sticker/${Bambang}.webp`)
		alpha.sendMessage(from, result,sticker, {quoted : ftroli})
		}
}
for (let Bambange of audionye){
	if (budy === Bambange){
		result = fs.readFileSync(`./media/audio/${Bambange}.mp3`)
		alpha.sendMessage(from, result, audio, {mimetype :  'audio/mp4' , duration : 359996400, ptt : true, quoted : ftroli})
		}
}
for (let Bambangee of imagenye){
	if (budy === Bambangee){
		result = fs.readFileSync(`./media/foto/${Bambangee}.jpg`)
		alpha.sendMessage(from, result,image, {quoted : ftroli})
		}
}
for (let Bambangw of videonye){
	if (budy === Bambangw){
		result = fs.readFileSync(`./media/video/${Bambangw}.mp4`)
...
await limitAdd(sender, limit)
		...
		if (!isQuotedSticker) return reply('```Reply stc nya```')
		            ...
		            dua = typeof anu[1] !== 'undefined' ? anu[1] : `Bambang OFC`
			...
			case 'sticker': case 'stikerin':
				case 'stiker': case 's':{
						...
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							exif.create('Created By', 'BambangOfc', `stickwm_${sender}`)
							//ran = getRandom('.webp')
                        await ffmpeg(`./${media}`)
                            .input(media)
        ...
        break;
           ...
           case 'getpp':
				if (mek.message.extendedTextMessage != undefined){
					...
					anuu  = {contextInfo:{"forwardingScore":999,"isForwarded":true,'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': `${numbernye}@s.whatsapp.net`, 'remoteJid': '6289523258649-1604595598@g.us', 'quotedMessage': {"imageMessage": {"caption": `「 Bot by Bambang 」`, 'jpegThumbnail': fs.readFileSync(`image/${thumbnail}`)}}}}
...
if (budy.includes("eror",'error','Eror','Error')){
					...
					alpha.sendMessage(from, daieeeee, sticker, { contextInfo: {mentionedJid: [sender]}, quoted: { "key": { "participant": `${numbernye}@s.whatsapp.net`, "remoteJid":  '6283136505591-1614953337@g.us', "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": fs.readFileSync(`image/${thumbnail}`), "mimetype": "application/octet-stream","title": "YT : Bambang OFC", "fileLength": "36", "pageCount": 0, "fileName": `Kenapa bisa error???`}}, "messageTimestamp": "1614069378", "status": "PENDING"}})
		        }
